...
public class BatchJob {
	...
	public static class Tokenizer implements FlatMapFunction<String, Tuple2<Character, Integer>>{
		@Override
		public void flatMap(String s, Collector<Tuple2<Character, Integer>> collector) throws Exception {
			char[] tokens = s.toLowerCase().replaceAll("\\p{Punct}","").replaceAll("\\p{Space}","").replaceAll("-","").replaceAll("[1-9]","").toCharArray();

			for(Character token:tokens){
				collector.collect(new Tuple2<Character, Integer>(token,1));
			}
		}
	}
}
