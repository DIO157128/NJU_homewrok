...
public class Cache {
    ...
    private int fetch(String pAddr) {
        int n=Log(setSize);
        int memoryblocknum=getBlockNO(pAddr);
        int linenum=map(memoryblocknum);
        if(linenum!=-1){
            return linenum;
        } else {
            String todo = pAddr.substring(0, 22) + "0000000000";
            char[] data = Memory.getMemory().read(todo, LINE_SIZE_B);
            cache.get(memoryblocknum % (LINE_SIZE_B / setSize) * setSize).validBit=true;
            cache.get(memoryblocknum % (LINE_SIZE_B / setSize) * setSize).tag=zeroadd(pAddr.substring(0, 12 + n)).toCharArray();
            cache.get(memoryblocknum % (LINE_SIZE_B / setSize) * setSize).data=data;
            return memoryblocknum % (LINE_SIZE_B / setSize) * setSize;
        }
    }
    ...
    private int map(int blockNO) {
        ...
        int numofgroups=1024/setSize;
        int startnum=blockNO%numofgroups*setSize;
        for(int i=startnum;i<startnum+setSize;i++){
