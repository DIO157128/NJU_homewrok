...
public class MMU {
    ...
    private String toRealLinearAddr(String logicAddr) {
        String high_16="000000000000"+logicAddr.substring(0,16)+"0000";
        String offset="0000000000000000"+logicAddr.substring(32,48);
        String physicalAddr=new ALU().add(new DataType(high_16),new DataType(offset)).toString();
        return physicalAddr;
    }
    ...
    private String toSegLinearAddr(String logicAddr) {
        int segIndex=getSegIndex(logicAddr);
        String base=new String(memory.getBaseOfSegDes(segIndex));
        String segOffset=logicAddr.substring(16,48);
        String LinearAddr=new ALU().add(new DataType(base),new DataType(segOffset)).toString();
        return LinearAddr;
    }
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        int vIndex=Integer.parseInt(transformer.binaryToInt(linearAddr.substring(0,20)));
        String voffset=linearAddr.substring(20,32);
        char[]pagefram;
        if (TLB.isAvailable) {
            int TLBindex=TLB.getTLB().TLBsearch(vIndex);
            if(TLBindex!=-1){
                pagefram=TLB.getTLB().getTLBPageFrame(TLBindex);
            }
            else pagefram=memory.getFrameOfPage(TLBindex);
        } else {
            pagefram=memory.getFrameOfPage(vIndex);
        }
        String pIndex=new String(pagefram);
        String pAddr=pIndex+voffset;
        return pAddr;
    }
