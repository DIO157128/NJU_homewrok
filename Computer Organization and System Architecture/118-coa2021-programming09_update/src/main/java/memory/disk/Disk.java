...
public class Disk {
    ...
    public static final int TRACK_NUM = 256;        // 磁道数,8
    public static final int SECTOR_PER_TRACK = 64;  // 每磁道扇区数,6
    public static final int BYTE_PER_SECTOR = 512;  // 每扇区字节数,9
    ...
    private static class DiskHead {
        ...
        public void seek(int start) {
            int actualstart=start%(int)Math.pow(2,23);
            track=actualstart/(int)Math.pow(2,15);
            actualstart=actualstart%(int)Math.pow(2,15);
            sector=actualstart/(int)Math.pow(2,9);
            actualstart=actualstart%(int) Math.pow(2,9);
            point=actualstart;
        }
