...
public class Memory {
    ...
    public void seg_load(int segIndex) {
        String pAddr=segindexDealer(segIndex);
        char[] data=disk.read(pAddr,1024*1024);
        if(!PAGE){
            write(transformer.intToBinary("0"),1024*1024,data);
        }
        SegDescriptor sg=getSegDescriptor(segIndex);
        sg.setSegDescriptor();
    }
    public String segindexDealer(int segIndex){
        String s1=new Transformer().intToBinary(String.valueOf(segIndex)).toString().substring(19,32);
        return s1+"00000000000000000000000000000000000";
    }//线性地址高13位为段址
    ...
    public void page_load(int vPageNo) {
        String pAddr=vPageNoDealer(vPageNo);
        char[] data=disk.read(pAddr,PAGE_SIZE_B);
        int vacntpage=checkValid();
        PageItem pg=getPageItem(vPageNo);
        pg.isInMem=true;
        String pgno=transformer.intToBinary(String.valueOf(vacntpage)).substring(12,32);
        pg.pageFrame=pgno.toCharArray();
        write(pgno+"000000000000",PAGE_SIZE_B,data);//先根据虚拟页号，查到要填的虚拟页框，然后找到空闲的物理页框，把物理页框号告诉虚拟页框，再把数据写到物理页框里面
    }
    public String vPageNoDealer(int vPageNo){
        String s1=new Transformer().intToBinary(String.valueOf(vPageNo));
        return s1.substring(12,32)+"000000000000";//虚拟地址：20位虚拟页号+12位偏移量
    }
    public int checkValid(){
        for(int i=0;i<MEM_SIZE_B / PAGE_SIZE_B;i++){
            if(!pageValid[i]){
                pageValid[i]=!pageValid[i];
                return i;
            }
        }
        return -1;
