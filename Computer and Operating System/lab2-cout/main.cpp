...
bool validJudge(vector<string>command,vector<string>param,vector<string>path){
    ...
    regex validparam("-l+");
    if(command.size()==0){
        cout<<"Inadequate command!"<<endl;
//        asmprint("Inadequate command!\n");
        return 0;
    }
    if(command.size()>1){
//        asmprint("Multiple commands!\n");
        cout<<"Multiple commands!"<<endl;
        return 0;
    }
    if(path.size()>1){
...
int main(){
    FILE* fat12;
    fat12 = fopen("E:\\OSshare\\lab2\\a.IMA", "r");	//打开FAT12的映像文件
    struct BPB bpb;
    ...
    while(true){
        ...
        for(int i =0;i<words.size();i++){
            ...
            else if(words[i].substr(0,1)==prefix_path){
                path.push_back(words[i]);
            } else{
                command.push_back(words[i]);
            }
