
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			      console.c
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
						    Forrest Yu, 2005
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/

/*
	回车键: 把光标移到第一列
	换行键: 把光标前进到下一行
*/


#include "type.h"
#include "const.h"
#include "protect.h"
#include "string.h"
#include "proc.h"
#include "tty.h"
#include "console.h"
#include "global.h"
#include "keyboard.h"
#include "proto.h"
PRIVATE void set_cursor(unsigned int position);
PRIVATE void set_video_start_addr(u32 addr);
PRIVATE void flush(CONSOLE* p_con);
/*======================================================================*
			   init_screen
 *======================================================================*/
PUBLIC void exit_esc(CONSOLE* p_con){
    u8* p_vmem = (u8*)(V_MEM_BASE);
    for(int i=p_con->search_pos;i<p_con->cursor;i++){
        *(p_vmem+2*i) = ' ';
        *(p_vmem+2*i+1) = DEFAULT_CHAR_COLOR;
    }
    for(int i=0;i<p_con->search_pos;i++){
        if(*(p_vmem+2*i+1)!=TAB_CHAR_COLOR){
            *(p_vmem+2*i+1) = DEFAULT_CHAR_COLOR;
        }

    }
    p_con->cursor = p_con->search_pos;
    p_con->pos_stack.ptr = p_con->pos_stack.search_ptr;
    p_con->out_char_stack.ptr = p_con->out_char_stack.search_ptr;
    flush(p_con); // 更新p_con，这个不能漏
}

PUBLIC void search(CONSOLE *p_con){
    int start_idx,end_idx; // 滑动窗口
    for(int i = 0; i < p_con->search_pos*2;i+=2){ // 遍历原始白色输入
        start_idx= end_idx = i; // 初始化窗口为0
        int flag = 1; // 是否匹配
        // 遍历匹配
        for(int j = p_con->search_pos*2;j<p_con->cursor*2;j+=2){
            if(*((u8*)(V_MEM_BASE+end_idx))==*((u8*)(V_MEM_BASE+j))){
                if(*((u8*)(V_MEM_BASE+end_idx))==' '){
                    if(*((u8*)(V_MEM_BASE+end_idx+1))==TAB_CHAR_COLOR&&*((u8*)(V_MEM_BASE+j+1))==RED){
                        flag =0;
                        break;
                    } else if(*((u8*)(V_MEM_BASE+end_idx+1))==TAB_CHAR_COLOR&&*((u8*)(V_MEM_BASE+j+1))==TAB_CHAR_COLOR){
                        end_idx+=2;
                    } else if(*((u8*)(V_MEM_BASE+end_idx+1))==DEFAULT_CHAR_COLOR&&*((u8*)(V_MEM_BASE+j+1))==TAB_CHAR_COLOR){
                        flag =0;
                        break;
                    }else if(*((u8*)(V_MEM_BASE+end_idx+1))==DEFAULT_CHAR_COLOR&&*((u8*)(V_MEM_BASE+j+1))==RED){
                        end_idx+=2;
                    }
                } else{
                    end_idx+=2;
                }
            }else{
                flag= 0;
                break;
            }
        }
        // 如果找到，标红
        if(flag==1){
            for(int j = start_idx;j<end_idx;j+=2){
                if(*(u8*)(V_MEM_BASE + j + 1) != TAB_CHAR_COLOR){
                    *(u8*)(V_MEM_BASE + j + 1) = RED;
                }
            }
        }
    }
}
PUBLIC void init_screen(TTY* p_tty)
{
	int nr_tty = p_tty - tty_table;
	p_tty->p_console = console_table + nr_tty;
    p_tty->p_console->pos_stack.ptr = 0;
	int v_mem_size = V_MEM_SIZE >> 1;	/* 显存总大小 (in WORD) */

	int con_v_mem_size                   = v_mem_size / NR_CONSOLES;
	p_tty->p_console->original_addr      = nr_tty * con_v_mem_size;
	p_tty->p_console->v_mem_limit        = con_v_mem_size;
	p_tty->p_console->current_start_addr = p_tty->p_console->original_addr;

	/* 默认光标位置在最开始处 */
	p_tty->p_console->cursor = p_tty->p_console->original_addr;

	if (nr_tty == 0) {
		/* 第一个控制台沿用原来的光标位置 */
		p_tty->p_console->cursor = disp_pos / 2;
		disp_pos = 0;
	}
	else {
		out_char(p_tty->p_console, nr_tty + '0');
		out_char(p_tty->p_console, '#');
	}

	set_cursor(p_tty->p_console->cursor);
}


/*======================================================================*
			   is_current_console
*======================================================================*/
PUBLIC int is_current_console(CONSOLE* p_con)
{
	return (p_con == &console_table[nr_current_console]);
}

PRIVATE void redo(CONSOLE *p_con){
    int start;
    if(mode==0){
        start = 0;
    }else if(mode==1){
        start = p_con->out_char_stack.search_ptr;
    }
    p_con->out_char_stack.ptr-=2; // z也被压栈了，所以要-=2而不是1
    if(p_con->out_char_stack.ptr<=start){
        p_con->out_char_stack.ptr=start;
        return;		// 如果已经清空了，直接返回，不操作
    }
    int i;
    for(i=start;i<p_con->out_char_stack.ptr;i++){
        out_char(p_con,p_con->out_char_stack.ch[i]);
    }
}
/*======================================================================*
			   out_char
 *======================================================================*/
PUBLIC void out_char(CONSOLE* p_con, char ch)
{
	u8* p_vmem = (u8*)(V_MEM_BASE + p_con->cursor * 2);

	switch(ch) {
	case '\n':
		if (p_con->cursor < p_con->original_addr +
		    p_con->v_mem_limit - SCREEN_WIDTH) {
            pushStack(p_con,p_con->cursor);
			p_con->cursor = p_con->original_addr + SCREEN_WIDTH * 
				((p_con->cursor - p_con->original_addr) /
				 SCREEN_WIDTH + 1);
		}
		break;
	case '\b':
		if (p_con->cursor > p_con->original_addr) {
            if(mode==1){
                if(p_con->cursor==p_con->search_pos){
                    break;
                }
            }
			int ori_pos = p_con->cursor;
            p_con->cursor = popStack(p_con);
            for(int i =0 ;i<ori_pos-p_con->cursor;i++){
                *(p_vmem-2-2*i) = ' ';
                *(p_vmem-1-2*i) = DEFAULT_CHAR_COLOR;
            }
		}
		break;
        case '\t':
            if (p_con->cursor < p_con->original_addr +
                                p_con->v_mem_limit - TAB_WIDTH) {
                for(int i=0;i<TAB_WIDTH;i++){
                    *p_vmem++ = ' ';
                    *p_vmem++ = TAB_CHAR_COLOR;
                }
                pushStack(p_con,p_con->cursor);
                p_con->cursor+=TAB_WIDTH;
            }
            break;
        case 'z':
            if(control&&(mode==0||mode==1)){ // mode=0或1下才可以进行撤销，逻辑不一样，只能撤销当前模式下的输入
                int temp; // 清屏开始的位置
                if(mode==0){
                    temp = 0;
                    // 初始化pos_stack的ptr指针
                    p_con->pos_stack.ptr = 0;
                }else if(mode==1){
                    temp = p_con->search_pos*2;
                    // 还原
                    p_con->pos_stack.ptr = p_con->pos_stack.search_ptr;
                }
                // clean and init screen
                disp_pos = temp;
                int i;
                for (i = 0 ; i < SCREEN_SIZE; ++i){
                    disp_str(" ");
                }
                disp_pos = temp;
                p_con->cursor = disp_pos / 2;
                flush(p_con);
                redo(p_con);
                return; // 撤销操作后直接返回
            }
        case 'Z':
            if(control&&(mode==0||mode==1)){ // mode=0或1下才可以进行撤销，逻辑不一样，只能撤销当前模式下的输入
                int temp; // 清屏开始的位置
                if(mode==0){
                    temp = 0;
                    // 初始化pos_stack的ptr指针
                    p_con->pos_stack.ptr = 0;
                }else if(mode==1){
                    temp = p_con->search_pos*2;
                    // 还原
                    p_con->pos_stack.ptr = p_con->pos_stack.search_ptr;
                }
                // clean and init screen
                disp_pos = temp;
                int i;
                for (i = 0 ; i < SCREEN_SIZE; ++i){
                    disp_str(" ");
                }
                disp_pos = temp;
                p_con->cursor = disp_pos / 2;
                flush(p_con);
                redo(p_con);
                return; // 撤销操作后直接返回
            }
	default:
		if (p_con->cursor <
		    p_con->original_addr + p_con->v_mem_limit - 1) {
			*p_vmem++ = ch;
			*p_vmem++ = mode==0?DEFAULT_CHAR_COLOR:RED;
            pushStack(p_con,p_con->cursor);
			p_con->cursor++;
		}
		break;
	}

	while (p_con->cursor >= p_con->current_start_addr + SCREEN_SIZE) {
		scroll_screen(p_con, SCR_DN);
	}

	flush(p_con);
}

PUBLIC void pushOutPutStack(CONSOLE* p_con,char ch){
    p_con->out_char_stack.ch[p_con->out_char_stack.ptr++]=ch;
}
/*======================================================================*
                           flush
*======================================================================*/
PRIVATE void flush(CONSOLE* p_con)
{
        set_cursor(p_con->cursor);
        set_video_start_addr(p_con->current_start_addr);
}

/*======================================================================*
			    set_cursor
 *======================================================================*/
PRIVATE void set_cursor(unsigned int position)
{
	disable_int();
	out_byte(CRTC_ADDR_REG, CURSOR_H);
	out_byte(CRTC_DATA_REG, (position >> 8) & 0xFF);
	out_byte(CRTC_ADDR_REG, CURSOR_L);
	out_byte(CRTC_DATA_REG, position & 0xFF);
	enable_int();
}

/*======================================================================*
			  set_video_start_addr
 *======================================================================*/
PRIVATE void set_video_start_addr(u32 addr)
{
	disable_int();
	out_byte(CRTC_ADDR_REG, START_ADDR_H);
	out_byte(CRTC_DATA_REG, (addr >> 8) & 0xFF);
	out_byte(CRTC_ADDR_REG, START_ADDR_L);
	out_byte(CRTC_DATA_REG, addr & 0xFF);
	enable_int();
}



/*======================================================================*
			   select_console
 *======================================================================*/
PUBLIC void select_console(int nr_console)	/* 0 ~ (NR_CONSOLES - 1) */
{
	if ((nr_console < 0) || (nr_console >= NR_CONSOLES)) {
		return;
	}

	nr_current_console = nr_console;

	set_cursor(console_table[nr_console].cursor);
	set_video_start_addr(console_table[nr_console].current_start_addr);
}

/*======================================================================*
			   scroll_screen
 *----------------------------------------------------------------------*
 滚屏.
 *----------------------------------------------------------------------*
 direction:
	SCR_UP	: 向上滚屏
	SCR_DN	: 向下滚屏
	其它	: 不做处理
 *======================================================================*/
PUBLIC void scroll_screen(CONSOLE* p_con, int direction)
{
	if (direction == SCR_UP) {
		if (p_con->current_start_addr > p_con->original_addr) {
			p_con->current_start_addr -= SCREEN_WIDTH;
		}
	}
	else if (direction == SCR_DN) {
		if (p_con->current_start_addr + SCREEN_SIZE <
		    p_con->original_addr + p_con->v_mem_limit) {
			p_con->current_start_addr += SCREEN_WIDTH;
		}
	}
	else{
	}

	set_video_start_addr(p_con->current_start_addr);
	set_cursor(p_con->cursor);
}

