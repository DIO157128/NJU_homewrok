...
public class WarehouseServiceImpl implements WarehouseService {
    ...
    private TimeFormatConverter trans;
...
//            warehouseDao.deductQuantity(warehousePO);
    ...
    public List<WarehouseIODetailPO> getWarehouseIODetailByTime(String beginDateStr, String endDateStr) {
         ...
         *  只读的接口不需要   @Transactional
         */
        List<WarehouseIODetailPO> warehouseIODetailByTime = warehouseInputSheetDao.getWarehouseIODetailByTime(trans.timeTransfer(beginDateStr), trans.timeTransfer(endDateStr));

        if(trans.checkTimeError(beginDateStr, endDateStr)) return null;
        return warehouseIODetailByTime;
    }
    ...
    public int getWarehouseInputProductQuantityByTime(String beginDateStr,String endDateStr){
         ...
         *   只读的接口不需要   @Transactional
         */
         if(warehouseInputSheetDao.getWarehouseInputProductQuantityByTime(trans.timeTransfer(beginDateStr), trans.timeTransfer(endDateStr)) == null) return 0;

        return warehouseInputSheetDao.getWarehouseInputProductQuantityByTime(trans.timeTransfer(beginDateStr), trans.timeTransfer(endDateStr));
    }
    ...
    public int getWarehouseOutProductQuantityByTime(String beginDateStr,String endDateStr){
         ...
         *   只读的接口不需要   @Transactional
         */
        if(warehouseOutputSheetDao.getWarehouseOutputProductQuantityByTime(trans.timeTransfer(beginDateStr), trans.timeTransfer(endDateStr))==null) return 0;

        return warehouseOutputSheetDao.getWarehouseOutputProductQuantityByTime(trans.timeTransfer(beginDateStr), trans.timeTransfer(endDateStr));
    }
