...
import com.nju.edu.erp.model.po.purcahseReturn.PurchaseReturnsSheetContentPO;
import com.nju.edu.erp.model.po.purcahseReturn.PurchaseReturnsSheetPO;
        ...
        if(state.equals(PurchaseReturnsSheetState.FAILURE)) {
            ...
            if(state.equals(PurchaseReturnsSheetState.SUCCESS)) {
                ...
                for (PurchaseReturnsSheetContentPO content:
                    ...
                    if(warehousePO.getQuantity() >= quantity) {         //否则没有足够的产品进行退回
                        ...
                        payableToDeduct = payableToDeduct.add(content.getUnitPrice().multiply(BigDecimal.valueOf(quantity))) ;  //σ单位进价*quantity
                    } else {
                ...
                //应用VO作customerService.updateCustomer的参数
                CustomerVO customer_VO = new CustomerVO();
                BeanUtils.copyProperties(customer, customer_VO);

                customer.setPayable(customer.getPayable().subtract(payableToDeduct));
                customerService.updateCustomer(customer_VO);                   //进货退回，本公司应付款金额减少
            }
        }
    }
}