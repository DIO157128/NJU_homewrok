...
<script>
import {createSaleReturn} from "@/network/sale";
...
export default {
  ...
  methods: {
    ...
    red(index) {
      ...
      newSheet.saleReturnsSheetContent = this.list.at(index).saleReturnsSheetContent
      for (let item of newSheet.saleReturnsSheetContent) {
        item.quantity = -1 * item.quantity;
      }
      newSheet.state = null
      createSaleReturn(newSheet).then(_res => {
        if (_res.msg === 'Success') {
