  ...
  <el-container style="height: 100%; border: 1px solid #eee">
    <el-aside width="250px" style="background-color: #545c64">
      ...
      >
        ...
        <el-submenu index="6"
                    v-if="permit(PATH.COMPANY_ACCOUNT_VIEW.requiresAuth)">
          <template slot="title">
            <i class="el-icon-receiving"></i>
            <span slot="title">财务管理</span>
          </template>
          <el-menu-item
              :index="PATH.COMPANY_ACCOUNT_VIEW.path"
              v-if="permit(PATH.COMPANY_ACCOUNT_VIEW.requiresAuth)"
          >
            <i class="el-icon-receiving"></i>
          ...
          <el-menu-item
              :index="PATH.PAY_VIEW.path"
              v-if="permit(PATH.PAY_VIEW.requiresAuth)"
          >
            <i class="el-icon-receiving"></i>
        ...
        <el-submenu index="5"
                    v-if="permit(PATH.PUNCH_IN_SHEET.requiresAuth)">
          <template slot="title">
            <i class="el-icon-receiving"></i>
            <span slot="title">打卡管理</span>
          </template>
          <el-menu-item
              :index="PATH.PUNCH_IN_SHEET.path"
              v-if="permit(PATH.PUNCH_IN_SHEET.requiresAuth)"
          >
            <i class="el-icon-receiving"></i>
          ...
          <el-menu-item
              :index="PATH.PUNCH_IN_SHEET_CONTENT.path"
              v-if="permit(PATH.PUNCH_IN_SHEET_CONTENT.requiresAuth)"
          >
            <i class="el-icon-receiving"></i>
            <span slot="title">员工打卡详情</span>
          </el-menu-item>
        </el-submenu>
        <el-submenu index="7"
          ...
          <el-menu-item
              ...
              v-if="permit(PATH.POST_VIEW.requiresAuth)"
          >
            <i class="el-icon-receiving"></i>
        ...
        <el-submenu index="9"
                    v-if="permit(PATH.PROMOTION_STRATEGY_ONLEVEL_VIEW.requiresAuth)">
          <template slot="title">
            <i class="el-icon-receiving"></i>
            <span slot="title">促销策略</span>
          </template>
          <el-menu-item
            :index="PATH.PROMOTION_STRATEGY_ONLEVEL_VIEW.path"
            v-if="permit(PATH.PROMOTION_STRATEGY_ONLEVEL_VIEW.requiresAuth)"
          >
            <i class="el-icon-receiving"></i>
            <span slot="title">顾客等级促销策略</span>
          </el-menu-item>
          <el-menu-item
            :index="PATH.PROMOTION_STRATEGY_ONAMOUNT_VIEW.path"
            v-if="permit(PATH.PROMOTION_STRATEGY_ONAMOUNT_VIEW.requiresAuth)"
          >
            <i class="el-icon-receiving"></i>
      ...
      <div class="logout-fix">
        ...
        <el-tooltip
            ...
            :content="getUserRole"
            placement="right"
            v-if="permit(PATH.PUNCH_IN.requiresAuth)"
        >
          <div class="logout-name" @click="punchIn()">打卡</div>
        </el-tooltip>
        <el-tooltip
            class="item"
            effect="light"
...
import {punchin} from "@/network/staff";
    ...
    punchIn() {
      punchin().then(_res => {
        if (_res.msg === 'Success') {
          ...
          this.$message.error('请勿重复打卡并向HR确认您的信息已被录入');
        }
      })
    },
    getUsername() {
