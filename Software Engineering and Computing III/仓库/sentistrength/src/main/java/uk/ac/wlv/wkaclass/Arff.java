...
public class Arff {
    ...
    /**
     * 主函数，进行文件识别、异常排查处理、进行机器学习.
     *
     * @param args
     */
    public void main(String[] args) {
        boolean[] bArgumentRecognised = new boolean[args.length];
        String sUnlabelledTextFile = "";       // 未标签的
        String sLabelledTextFile = "";         // 已标签的
        String sArffFileIn = "";               // .arff文件输入
        String sTextFileOut = "";              // .txt文件输出
        String sClassifier = "smo";            // 分类器
        int iNGrams = 3;
        int iMaxFeatures = 0;
        int iClassType = 4;
        int iClassFor0 = 0;
        int iMinFeatureFrequency = 1;
        boolean bCompleteProcessing = false;   // 是否开启
        overallHelp();
        ...
        if (bCompleteProcessing) {
            ...
            if (iClassType == 4) {
                ...
                sClassifiedUnlabelledTextFile = FileOps.sChopFileNameExtension(sClassifiedUnlabelledArff) + "_Nout.txt";
                ...
                sMergedTextFile = FileOps.sChopFileNameExtension(sClassifiedUnlabelledTextFile) + "_Nmerged.txt";
                ...
                sClassifiedUnlabelledTextFile = FileOps.sChopFileNameExtension(sClassifiedUnlabelledArff) + "_Pout.txt";
                System.out.println("convertArffToText " + sClassifiedUnlabelledArff + " -> " + sClassifiedUnlabelledTextFile);
                System.out.println();
                convertArffToText(sClassifiedUnlabelledArff, sClassifiedUnlabelledTextFile);
                sMergedTextFile = FileOps.sChopFileNameExtension(sClassifiedUnlabelledTextFile) + "_Pmerged.txt";
                System.out.println("mergeLabelledAndUnlabelledTextFiles " + sClassifiedUnlabelledTextFile + ", " + sUnlabelledTextFile + " -> " + sMergedTextFile);
                mergeLabelledAndUnlabelledTextFiles(sClassifiedUnlabelledTextFile, sUnlabelledTextFile, sMergedTextFile);
            } else {
                ...
                sClassifiedUnlabelledTextFile = FileOps.sChopFileNameExtension(sClassifiedUnlabelledArff) + "_out.txt";
                System.out.println("convertArffToText " + sClassifiedUnlabelledArff + " -> " + sClassifiedUnlabelledTextFile);
                System.out.println();
                convertArffToText(sClassifiedUnlabelledArff, sClassifiedUnlabelledTextFile);
                sMergedTextFile = FileOps.sChopFileNameExtension(sClassifiedUnlabelledTextFile) + "_merged.txt";
                System.out.println("mergeLabelledAndUnlabelledTextFiles " + sClassifiedUnlabelledTextFile + ", " + sUnlabelledTextFile + " -> " + sMergedTextFile);
                mergeLabelledAndUnlabelledTextFiles(sClassifiedUnlabelledTextFile, sUnlabelledTextFile, sMergedTextFile);
            }
    ...
    /**
     * 将 未标记的txt文件 转换为 基于标记的txt文件的arff文件
     *
     * @param sLabelledTextFile
     * @param iClassType
     * @param iNGrams
     * @param iMinFeatureFrequency
     * @param iMaxFeatures
     * @param sUnlabelledTextFile
     * @return
     */
    private static String[] convertUnlabelledTextFileToArffBasedOnLabelledTextFile(String sLabelledTextFile, int iClassType, int iNGrams, int iMinFeatureFrequency, int iMaxFeatures, String sUnlabelledTextFile) {
        ...
        if (iMaxFeatures > 0) {
            ...
            for (i = 0; i < iLabelledArffFileCount; ++i) {
                sLabelledArffFilesReduced[i] = FileOps.sChopFileNameExtension(sLabelledArffFiles[i]) + " " + iMaxFeatures + ".arff";
    ...
    private static boolean writeArffFromIndex(String sSentiTextFileIn, StringIndex stringIndex, boolean bHeaderLine, TextParsingOptions textParsingOptions, ClassificationOptions classOptions, ClassificationResources resources, int iSentimentType, int iMinFeatureFrequency, String sArffFileOut, boolean bArffIndex) {
        ...
        try {
            ...
            while (true) {
                ...
                Paragraph para = new ParagraphFactory().createParagraph(classOptions.igEmotionParagraphCombineMethod);
                if (iSentimentType == 4) {
                    sData = sLine.split("\t");
                    if (sData.length > 2) {
    ...
    public static String ngramFileName(String sSentiTextFileIn, int iNgram) {
        return FileOps.sChopFileNameExtension(sSentiTextFileIn) + "_" + iNgram + ".arff";
    }

    public static String oneToNgramFileName(String sSentiTextFileIn, int iNgram) {
        return FileOps.sChopFileNameExtension(sSentiTextFileIn) + "_1-" + iNgram + ".arff";
    }

    public static String ngramFileNamePosNeg(String sSentiTextFileIn, int iNgram, boolean bPos) {
        return bPos ? FileOps.sChopFileNameExtension(sSentiTextFileIn) + "_" + iNgram + "pos.arff" : FileOps.sChopFileNameExtension(sSentiTextFileIn) + "_" + iNgram + "neg.arff";
    }

    public static String oneToNgramFileNamePosNeg(String sSentiTextFileIn, int iNgram, boolean bPos) {
        return bPos ? FileOps.sChopFileNameExtension(sSentiTextFileIn) + "_1-" + iNgram + "pos.arff" : FileOps.sChopFileNameExtension(sSentiTextFileIn) + "_1-" + iNgram + "neg.arff";
    }
    ...
    public static void convertArffToTextMultiple(String[] sArffIn, int iArffInCount, String[] sTextOut) {
        for (int i = 0; i < iArffInCount; ++i) {
            sTextOut[i] = FileOps.sChopFileNameExtension(sArffIn[i]) + " out.txt";
    ...
    public static void makeArffsWithTopNAttributes(String[] sArffIn, int iArffInCount, int iTopNAttributes, String[] sArffOut) {
        for (int i = 0; i < iArffInCount; ++i) {
            sArffOut[i] = FileOps.sChopFileNameExtension(sArffIn[i]) + " " + iTopNAttributes + ".arff";
