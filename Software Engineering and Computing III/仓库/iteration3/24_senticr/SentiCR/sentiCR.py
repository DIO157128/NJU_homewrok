...
# 填充emodict 和 contractions_dict两个列表
...
class SentiCR:
    # algo指定模型
    def __init__(self, algo="MIX", training_data=None):
        self.algo = algo
    ...
    def create_model_from_training_data(self):
        ...
        smote_model = SVMSMOTE(sampling_strategy=0.5, random_state=None, k_neighbors=15, svm_estimator=None)
    ...
    def read_data_from_oracle(self):
        workbook = load_workbook("oracle.xlsx")
...
def find_same_var(a, b, c):
    if a == b or b == c or a == c:
        return a if a == b or a == c else b
    return a
...
if __name__ == '__main__':
    ...
    os.makedirs(f"logs/reproduction", exist_ok=True)
    save_path = "reproduction/fisrt"
    ...
    for k in range(0, REPEAT):
        ...
        logger.info("F-measure:" + str(f1score))
    ...
    logger.info("-------------------------")
    logger.info("Average Precision: {}".format(mean(Precision)))
    logger.info("Average Recall: {}".format(mean(Recall)))
    logger.info("Average Fmean: {}".format(mean(Fmean)))
    logger.info("Average Accuracy: {}".format(mean(Accuracy)))
    logger.info("-------------------------")
    # print("-------------------------")
